...
#SBATCH --mail-user tomleegolf@yahoo.com.au
...
includephase=unphased  # Options are phased or unphased


#vcf=/mnt/hcs/WCHP_Clinical_Genetics/SequenceData/GRCh37/VariantCalls/Nexteraexome37Mb/20180518_Nextera37Mb_VariantCalls/20180518_Nextera37Mb_VariantCalls_Split_Annotated_4/20180518_Nextera37Mb_VariantCalls_Split_ann.vcf.gz #Creates a vcf variable that indicates which vcf file to use
ped=/mnt/hcs/WCHP_Clinical_Genetics/SequenceData/Meta/Ped.txt # Creates a variable that indicates the file that shows pedigree information for the cohort
disorders=/resource/domains/STUDENT/leeto433/diseases3.txt # Creates a variable that indicates the file that has the information on the diseases that we are looking for
...
project=${SCRATCH}/$(basename ${vcf} .vcf.gz)_SecondaryFindings # Creates a variable so that we can create a directory that is located in the scratch folder of the person running the script, and removes the .vcf.gz suffix and appends _SecondaryFindings onto the end of the directory name
mkdir -p ${project} # Creates the directory named as above. 'mkdir -p' means make parent directories as needed
...
# Tests if there are any words in the selectedsamples.list file. If there are no words in the file, ie no sample IDs have been specified,
# then analyse all samples. Otherwise only analyse samples that have been specified in the samples variable
if [ $(wc -w < ${project}/selectedsamples.list) -lt 1 ]; then
	inputsamples=${project}/allsamples.list	
else
	inputsamples=${project}/selectedsamples.list
fi

module purge # Clear any modules that may already have been loaded. Some modules may interfere with BCFtools
module load BCFtools # BCFtools is a program that allows us to work with vcf files
...
if [ ! -f $project/allsamples.list ]; then
	$(which bcftools) query -l ${vcf} > $project/allsamples.list # query -l prints list of sample IDs only
fi

while read -u 3 -r subject; do # -r flag prevents backslash character from being interpreted. -u 3 means receive input from channel 3
	
	# Updates jobname on Slurm so we can see which subject we are processing
	scontrol update jobid=${SLURM_JOB_ID} jobname=Secondary_${subject}	
	
	mkdir -p ${project}/${subject} # makes a folder for each subject
	cd ${project}/${subject}
	mkdir -p slurm # creates a folder for slurm output
	...
	if [ ! -f ${project}/${subject}/subset.vcf.gz ]; then
		$(which bcftools) view -Oz -o ${project}/${subject}/subset.vcf.gz -s ${samples} ${vcf} && $(which bcftools) index ${project}/${subject}/subset.vcf.gz
	fi
	...
	while read -u 3 -r diseasegene;do
		...
		# Updates job name on Slurm so we can see which subject and gene we are processing
		scontrol update jobid=${SLURM_JOB_ID} jobname=Secondary_${subject}_${hgnc_symbol}
		
		mkdir -p ${project}/${subject}/${category}/${hgnc_symbol}/slurm
